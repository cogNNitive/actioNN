...
cp -r nn-dev-opencode-model-router ~/.agents/skills/

# Or for OpenCode specifically:
cp -r nn-dev-opencode-model-router ~/.config/opencode/skills/
```
...
> Install the `nn-dev-opencode-model-router` skill and add the following instruction to my agent configuration (AGENTS.md, CLAUDE.md, or equivalent):
>
> ```
> At the start of every session (first user message), load the nn-dev-opencode-model-router skill and follow its Rules of Engagement. This check runs once per session. Do not re-evaluate unless I ask.
> ```
>
