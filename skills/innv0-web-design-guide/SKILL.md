...
> **"Does this project have a separate documentation site (e.g. Docsify) that should live at `/documentation/`?"**

If yes, generate the Documentation section at `docs/documentation/` with Docsify. This is entirely optional — the skill works without it.

Then ask:

> **"Does this project include a separate application (SPA, web app, etc.) that should be served from `/app/`?"**

If yes, create an empty placeholder at `docs/app/index.html` with a redirect note, then follow the [App Integration](#-app-integration-optional) section for configuration guidance. This is entirely optional — the skill works without it.
...
The core structure is always the same; optional additions (documentation, app) add subdirectories under `docs/` without changing anything else:

```
repo/
├── docs/                    ← GitHub Pages publishing source (becomes web root)
│   ├── index.html           ← Web pública (siempre presente)
│   ├── index.md             ← Markdown twin (siempre presente)
│   ├── about.html           ← /about.html
│   ├── about.md             ← /about.md
│   ├── robots.txt           ← /robots.txt
│   ├── llms.txt             ← /llms.txt
│   ├── llms-full.txt        ← /llms-full.txt (opcional)
...
│   ├── ...
│   │
│   ├── documentation/       ← Docsify (OPCIONAL — solo si el proyecto tiene docs separadas)
│   │   ├── index.html
│   │   ├── README.md
│   │   └── ...
│   │
│   └── app/                 ← SPA compilada (OPCIONAL — solo si el proyecto tiene una app separada)
│       └── index.html
│
...
**Regla:** `docs/` contiene la web pública en su raíz. Todo lo demás son subdirectorios opcionales. El skill funciona igual con o sin ellos — no hay pasos obligatorios que dependan de `documentation/` o `app/`.
...
If your project includes a **separate application** (SPA, Vue, React, vanilla JS app, etc.) that lives outside `docs/` and gets built or compiled separately, this section describes how to integrate it with the web pública structure.

**The skill does NOT assume you have an app.** Skip this section entirely if you don't. If you do, the app lives at `docs/app/` and is served from `https://site.com/app/`.
...
repo/
├── docs/
│   ├── index.html           ← Web pública (sin cambios)
│   ├── app/                 ← App compilada (built output)
│   │   └── index.html
│   └── ...
├── src/                     ← Código fuente de la app (si aplica, no se despliega)
├── package.json             ← Dependencias de la app (si aplica)
├── vite.config.ts           ← Config de build (si aplica)
└── .github/workflows/       ← CI/CD (si aplica)
```

The `docs/app/` directory contains the **compiled/built** application. Its content is generated by your build tool — the skill does not author it.

### Build tool configuration

If your app uses a build tool (Vite, webpack, etc.), configure it to output to `docs/app/`:

- **Vite**: set `base: '/app/'` in `vite.config.ts` so asset paths resolve correctly under the `/app/` subpath.
- **Other tools**: ensure the base/public path is set to `/app/` so all assets load from the correct URL.

### No build? No problem

If your app has no build step (plain HTML/JS/CSS), place the files directly in `docs/app/` and reference them with relative or root-relative paths starting with `/app/`.

### Dev server considerations

During development, the app and the web pública are typically served by different tools:

- The **web pública** can be previewed with a static server: `python -m http.server 8080 --directory docs`
- The **app** may have its own dev server (e.g. `npm run dev` on a different port)
- Alternatively, configure your build tool's dev server to proxy or serve the web pública alongside the app

### CI/CD

If your project has a CI/CD pipeline that builds the app:

1. Run the build step (e.g. `npm run build`)
2. Copy or output the build result to `docs/app/`
3. Deploy `docs/` as the GitHub Pages artifact

Example GitHub Actions snippet (adapt to your stack):

```yaml
- run: npm run build
- run: rm -rf docs/app && mkdir -p docs/app && cp -r dist/* docs/app/
- uses: actions/upload-pages-artifact@v3
  with:
    path: docs
```

If there is no CI/CD, just build locally and commit `docs/app/` (or use a `.gitignore` entry if you prefer not to track it).

### Navigation

The web pública should link to the app. Add a link like this to the web pública's navigation:

```html
<a href="/app/" target="_blank" rel="noopener noreferrer">Open App</a>
```

This is a standard external-style link — the app is conceptually separate even though it lives on the same domain.
...
### If you have an app (optional)

These steps only apply if your project includes a separate application:

5. **Build the app** (if applicable): Run your build command (e.g. `npm run build`) so the compiled output ends up in `docs/app/`
6. **Verify `docs/app/index.html`** exists before pushing — without it, `/app/` will return 404
7. **If using CI/CD**, ensure your pipeline builds the app and copies the output to `docs/app/` before uploading the Pages artifact
8. **If building locally**, commit `docs/app/` or add it to `.gitignore` and regenerate on each deploy
