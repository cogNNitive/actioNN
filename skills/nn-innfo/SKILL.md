...
### Step 1 — Check availability (AUTHORITATIVE)
...
Do NOT check config files first — check the actual session tools. **This is the only authoritative check.**

If the tools are listed → proceed. The MCP Operating Model (§1) applies. Stop here.

### Step 2 — If NOT available, diagnose

Tools not found → they may be registered but not yet loaded. Check:

1. **Global OpenCode config**: `~/.config/opencode/opencode.json` — look for `innfo-mcp` under `mcp`
2. **Local repo config**: `.opencode/opencode.json` — look for `innfo-mcp` under `mcp`
...
If found in any config → ask the user to reload/restart OpenCode so the session picks up the MCP registration.

### Step 3 — Not configured anywhere
...
### Step 4 — If MCP is still unavailable after activation
...
**Naming convention** (defiNNe §6): Model files MUST follow `<Name>_V_x-y-z_<Template>_NN.md`. Draft status goes in frontmatter (`status: "Draft"`), never in filename (`_NN_draft.md` is INVALID).
...
| `enum` | Fixed set of 3–10 predefined values | Colored badge / dropdown filter |
...
> "`markdown_inline` for `strengths` because source values contain multi-line bullet lists — `string` would render raw `["item1", "item2"]` JSON in the table."
...
### Step 4 — Execute with `apply_change`

Once confirmed, use `apply_change({ id, op: "add_field", args })` for each field. Do NOT hand-edit the YAML — the MCP validates and rejects invalid changes.
...
1. **NEVER modify** a published template spec in `specs/` — they are immutable.
2. **NEVER modify** a `.specs/` local copy that Cognitive downloaded from a URL — it is a cache, not a workspace file.
3. **NEVER modify** the original template's `concepts[]`, `markers[]`, or `matrices[]` in place.

### Specialization Procedure

**Step 1 — Identify the template to extend**

The model's `parent_spec.name` tells you which template it uses.

**Step 2 — Fetch the template content**

Use `get_template({ model_id })` to resolve the template structure.

**Step 3 — Create the specialization file**

Naming: `<ModelName>_<Template>_V_x-y-z_spec_NN.md`

Examples:
- `GG_business_V_1-0-0_spec_NN.md` — specialization of business template for the GG model
- `Ghostbusters_business_V_0-1-2_spec_NN.md` — specialization for Ghostbusters

Place it at the project root or in a `specs/` directory alongside the model.

**Step 4 — Specialization frontmatter**
...
title: "<ModelName> — <Template> Specialization"
...
Run `validate_model({ id })` — the MCP resolves the specialization as a level-2 template.
...
This protocol applies AFTER every model or spec edit — whether from §8, §9, §5 (Rename), or any other modification.
...
- `valid: true` → proceed
- `valid: false` → list `errors[]` and `warnings[]`. Do NOT present the model as valid. Offer to fix or revert.
...
9. **Field Creation Protocol**: When creating fields, follow §8 — propose before executing, get user confirmation, validate after.
10. **Specialization over Mutation**: When a model needs custom fields beyond the base template, follow §9 — create a specialization rather than modifying the original template.
11. **Post-Edit Validation**: After any model/spec edit, run `validate_model()` and ask about version bump (§10). Never declare completion without validation.
