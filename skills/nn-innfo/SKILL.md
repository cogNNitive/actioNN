...
3. **Gramática exacta:**
   ...
   <ancla> ::= L<n> | L<n>-L<m>
   ```
   - `<ref>` is SIEMPRE una ruta que empieza con `sources/nn/` and ends in `.md` — es la misma ruta que el archivo normalizado, nunca una ruta a `sources/original/` ni al documento fuente sin normalizar.
...
5. **Opción Recomendada Primero:** Prefijar la opción `[a]`/`[1]` con `(Recomendado)`/`(Recomendada)`.
6. **Notación de Selección Múltiple:** Cuando las opciones no son mutuamente excluyentes, incluir *"Podés seleccionar una opción o una combinación (ej. A y B)"*.
